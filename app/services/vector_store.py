...
class VectorStoreService:
    ...
    def create_vector_store(self) -> FAISS:
        ...
        logger.info("[vector] Total document to index: %d (learning: %d, chat: %d)",
        ...
        if not all_documents:
            ...
            logger.info("[VECTOR] FAISS index built successfully with %d, overlap=%d", len(chunks))
    ...
    def get_retriever(self, k: int = 10):
        if not self.vector_store:
            raise RuntimeError("vector store not initialised. this shold not happen.")
