...
class ChatService:
    ...
    def get_or_create_session(self, session_id: Optional[str] = None) -> str:
        ...
        if not self.validate_session_id(session_id):
            raise ValueError(
                f"Invalid session_id format: {session_id}. session ID must not be empty, "
                "must not contain path traversal characters, and must be under 255 characters"
            )

        if session_id in self.sessions:
            logger.info("[TIMING] session_get_or_create: %.3fs (existing)", time.perf_counter() - t0)
            return session_id

        if self.load_session_from_disk(session_id):
            logger.info("[TIMING] session_get_or_create: %.3fs (loaded)", time.perf_counter() - t0)
            return session_id

        self.sessions[session_id] = []
        logger.info("[TIMING] session_get_or_create: %.3fs (created)", time.perf_counter() - t0)
    ...
    def process_message(self, session_id: str, user_message: str) -> str:
        ...
        logger.info("[GENERAL] response lenght:%d chars | preview: %.120s", len(response), response)
        return response
    ...
    def process_message_stream(
        ...
        logger.info("[GENERAL] session: %s | usser:%200s", session_id[:12], user_message)
        self.add_message(session_id, "user", user_message)
        ...
        try:
            ...
            logger.info("[GENERAL-STREAM] Completed | chunks: %d | response lenght :%d chars", chunk_count, len(final_response))
            self.save_chat_session(session_id)
