/* ================================================================
   I.D.R.I.S Frontend — Main Application Logic
   ================================================================
   ARCHITECTURE OVERVIEW
   ---------------------
   This file powers the entire frontend of the I.D.R.I.S AI assistant.
   It handles:
   ...
   4. ANIMATED ORB — A WebGL-powered visual orb (rendered by a
      ...
      "activates" when I.D.R.I.S is speaking and goes idle otherwise.
 ...
 * (e.g., "https://IDRIS.example.com"). During local development, it falls
...
function bindEvents() {
  ...
  document.querySelectorAll(".chip").forEach((c) => {
    c.addEventListener("click", () => {
      if (!isStreaming) sendMessage(c.dataset.msg);
    });
  });
...
/* ================================================================
   ...
   consists of:
     - An avatar circle ("J" for IDRIS, "U" for user)
...
function isUrlLike(str) {
  ...
  return (
    s.length > 40 &&
    (/^https?:\/\//i.test(s) || /\%2f|\%3a|\.com\/|\.org\//i.test(s))
  );
}
...
function friendlyUrlLabel(url) {
  ...
  try {
    ...
    const path =
      u.pathname !== "/"
        ? u.pathname.slice(0, 20) + (u.pathname.length > 20 ? "…" : "")
        : "";
...
function renderSearchResults(payload) {
  ...
  if (searchResultsQuery)
    searchResultsQuery.textContent = (payload.query || "").trim() || "Search";
  if (searchResultsAnswer)
    searchResultsAnswer.textContent = (payload.answer || "").trim() || "";
 ...
 *     <div class="msg-body">
 *       <div class="msg-label">IDRIS (General) | You</div>
 *       <div class="msg-content">...text...</div>
 *     </div>
 *   </div>
 */
/* Inline SVG icons for chat avatars (user = person, assistant = bot). */
const AVATAR_ICON_USER =
  '<svg class="msg-avatar-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>';
const AVATAR_ICON_ASSISTANT =
  '<svg class="msg-avatar-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="10" rx="2"/><circle cx="12" cy="5" r="2"/><path d="M12 7v4"/><circle cx="9" cy="16" r="1" fill="currentColor"/><circle cx="15" cy="16" r="1" fill="currentColor"/></svg>';
...
function addMessage(role, text) {
  ...
  avatar.innerHTML =
    role === "assistant" ? AVATAR_ICON_ASSISTANT : AVATAR_ICON_USER;
  ...
  label.textContent =
    role === "assistant"
      ? `IDRIS  (${currentMode === "realtime" ? "Realtime" : "General"})`
      : "You";
...
function addTypingIndicator() {
  ...
  const label = document.createElement("div");
  label.className = "msg-label";
  label.textContent = `IDRIS  (${currentMode === "realtime" ? "Realtime" : "General"})`;
...
async function sendMessage(textOverride) {
  ...
  try {
    ...
    const placeholder =
      currentMode === "realtime" ? "Searching..." : "Thinking...";
